...
* [Reviewing Pull Requests](#reviewing-pull-requests)
  ...
  * [Accept that there are different opinions about what belongs in DroneAid](#accept-that-there-are-different-opinions-about-what-belongs-in-droneaid)
...
DroneAid does not yet have a style guide.
...
Fixes: https://github.com/Code-and-Response/DroneAid/issues/1337
Refs: http://eslint.org/docs/rules/space-in-parens.html
```

If you are new to contributing to DroneAid, please try to do your best at
...
at least two DroneAid Collaborators (one Collaborator approval is enough if the
...
All DroneAid contributors who choose to review and provide feedback on Pull
...
1. Does this change make sense for DroneAid?
2. Does this change make DroneAid better, even if only incrementally?
...
DroneAid Collaborator landing the Pull Request but they can also be an
...
a particular change that makes DroneAid better, but the individual might just
not want to have anything to do with DroneAid ever again. The goal is not just
having good code.
...
### Accept that there are different opinions about what belongs in DroneAid

Opinions on this vary, even among the members of the Technical Steering
Committee.

One general rule of thumb is that if DroneAid itself needs it (due to historic
or functional reasons), then it belongs in DroneAid.
...
make DroneAid better. These may or may not make sense to add, but as with all
...
DroneAid has always optimized for speed of execution. If a particular change
can be shown to make some part of DroneAid faster, it's quite likely to be
...
That said, performance is not the only factor to consider. DroneAid also
...
to make sure that it works on the platforms that DroneAid
